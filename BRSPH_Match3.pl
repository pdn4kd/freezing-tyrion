...
open my $output0, ">", "BRSPHmatchlines461.csv" or die "cannot open BRSPHmatch.csv: $!";
print $output0 "BSPHnyuID,RSPHnyuID\r\n";
open my $output1, ">", "BRSPHmatchcount461.csv" or die "cannot open BRSPHmatchcount.csv: $!";
...
while (my $row = $csv0->getline_hr($input0)) {
	open my $input1, "<", "RSPH.csv" or die "cannot open RSPH.csv: $!";
	...
	while (my $row1 = $csv1->getline_hr($input1)) {
		if ((abs($z - $row1->{zgal}) <= 0.01) && (abs($mstar - $row1->{'logMstar(Msun/h2)'}) <= 0.4) && (abs($mhalo - $row1->{'logMhalo2(Msun/h)'}) <= 0.6)) {
