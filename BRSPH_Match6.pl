...
open my $output0, ">", "5BRSPHmatchlines115cz.csv" or die "cannot open BRSPHmatch.csv: $!";
print $output0 "BSPHnyuID,RSPHnyuID\r\n";
open my $output1, ">", "5BRSPHmatchcount115cz.csv" or die "cannot open BRSPHmatchcount.csv: $!";
...
while (my $row = $csv0->getline_hr($input0)) {
	open my $input1, "<", "RSPHgz.csv" or die "cannot open RSPHgz.csv: $!";
	...
	my $rank = $row->{rank2};
	while ((my $row1 = $csv1->getline_hr($input1)) && ($match_count < 5)) {
		if ((abs($z - $row1->{zgal}) <= 0.005) && (abs($mstar - $row1->{'logMstar(Msun/h2)'}) <= 0.1) && (abs($mhalo - $row1->{'logMhalo2(Msun/h)'}) <= 0.1) && ($rank == $row1->{rank2})) {
